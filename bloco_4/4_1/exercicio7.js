// Escreva um programa que converte uma nota dada em porcentagem 
// (de 0 a 100) em conceitos de A a F. Siga essas regras:
// Porcentagem >= 90 -> A
// Porcentagem >= 80 -> B
// Porcentagem >= 70 -> C
// Porcentagem >= 60 -> D
// Porcentagem >= 50 -> E
// Porcentagem < 50 -> F
// O programa deve retornar uma mensagem de erro e 
// encerrar se a nota passada for menor que 0 ou maior que 100.

let nota = 92
if (nota < 0 || nota > 100) {
  console.log('Nota inválida! Verifique o valor computado...')
} else  if (nota >= 90 && nota <= 100) {
  console.log('Parabéns! Você atingiu o conceito máxima: A!')
} else if (nota >= 80 && nota < 90) {
  console.log('Seu resultado foi satisfatório! Conceito B.')
} else if (nota >= 70 && nota < 80) {
  console.log('Você está na média, conceito C')
} else if (nota >= 60 && nota < 70) {
  console.log('Conceito D')
} else if (nota >= 50 && nota < 60) {
  console.log('Conceito E')
} else {
  console.log('Conceito F')
}